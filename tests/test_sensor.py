    ...
    for sensor in SENSORS:
        ...
        if expected_value is None:
            assert state.state == "unknown", (
                f"Expected unknown state for {entity_id}, got {state.state}"
            )
        else:
        ...
        if sensor.get("device_class"):
            assert (
                state.attributes.get("device_class") == sensor["device_class"]
            )
        if sensor.get("state_class"):
            assert (
                state.attributes.get("state_class") == sensor["state_class"]
            )
